...
public class TransactionController {
	...
	public ResponseEntity<APIResponse<OrderProduct>> removeOrderProduct(@RequestParam String oid, @RequestParam String pid) {
		boolean isDeleted = orderProductService.deleteOrderProduct(oid, pid);
		
		if (!isDeleted) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new APIResponse<>(HttpStatus.NOT_FOUND.value(), "Failed to delete", null));
		}
